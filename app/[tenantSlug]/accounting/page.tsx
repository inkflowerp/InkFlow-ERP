...
export default function AccountingPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
            {EXPENSE_CATEGORIES.map((c) => (
              <Button
                ...
                className="text-xs h-8 px-3 whitespace-nowrap bangla-text"
              >
      ...
      {activeTab === 'cash_book' && (
        <div className="space-y-4">
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 flex justify-between items-center text-xs">
            <div>
      ...
      {activeTab === 'bank' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button size="sm" onClick={() => setIsNewBankOpen(true)} className="text-xs">
              <Plus className="mr-1.5 h-3.5 w-3.5" />
              Add Bank Account
            </Button>
          </div>
      ...
      {activeTab === 'profit' && (
        <Card className="p-6 space-y-6">
          ...
          <div className="space-y-3 font-mono text-xs max-w-2xl mx-auto">
            {/* 1. Gross Sales */}
            <div className="p-3.5 rounded-xl bg-slate-100 dark:bg-slate-900 flex justify-between items-center font-bold text-sm">
              <span className="text-slate-900 dark:text-white">1. Gross Billed Sales Revenue</span>
              <span className="text-blue-600">৳ {formatBDT(profitWaterfall.gross_sales)}</span>
            </div>

            {/* Deductions */}
            <div className="pl-6 space-y-2 border-l-2 border-slate-300 dark:border-slate-700">
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>(-) Raw Material Substrate Cost:</span>
                <span className="text-red-500 font-bold">- ৳ {formatBDT(profitWaterfall.material_cost)}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>(-) Direct Factory Machine Labor:</span>
                <span className="text-red-500 font-bold">- ৳ {formatBDT(profitWaterfall.labor_cost)}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>(-) Transport & Transit Fuel:</span>
                <span className="text-red-500 font-bold">- ৳ {formatBDT(profitWaterfall.delivery_cost)}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>(-) Site Installation Rigging:</span>
                <span className="text-red-500 font-bold">- ৳ {formatBDT(profitWaterfall.installation_cost)}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>(-) Operating Overhead Expenses (Rent, Power, Office):</span>
                <span className="text-red-500 font-bold">- ৳ {formatBDT(profitWaterfall.operating_expenses)}</span>
              </div>
            </div>

            {/* Total Estimated Net Profit */}
            <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border-2 border-emerald-500 flex justify-between items-center text-base font-black">
              ...
              <div className="text-2xl text-emerald-700 dark:text-emerald-300 font-black font-mono">
      ...
      >
        <form onSubmit={handleRecordCash} className="space-y-4 pt-1">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <Button type="button" variant="outline" onClick={() => setIsCashEntryOpen(false)}>
              Cancel
            </Button>
      ...
      >
        <form onSubmit={handleAddBank} className="space-y-4 pt-1">
          <div className="space-y-1.5">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <Button type="button" variant="outline" onClick={() => setIsNewBankOpen(false)}>
              Cancel
            </Button>
