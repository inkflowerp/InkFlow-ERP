...
export function isTransactionalKey(key: string): boolean {
  return (
    ...
    key === STORAGE_KEYS.PRINTING_METHODS ||
    key === STORAGE_KEYS.MATERIAL_PURCHASE_CONFIGS ||
    key === STORAGE_KEYS.FINISHING_OPTIONS ||
    key === STORAGE_KEYS.ADDITIONAL_OPTIONS ||
    key === STORAGE_KEYS.INSTALLATION_OPTIONS ||
    key === STORAGE_KEYS.CUSTOMER_RATES ||
    key === STORAGE_KEYS.PRICING_RULES ||
    key === STORAGE_KEYS.PRICE_OVERRIDES ||
    key === STORAGE_KEYS.PRODUCT_SUPPLIER_PRICES ||
    key === STORAGE_KEYS.MACHINERIES ||
    key === STORAGE_KEYS.MACHINERY_ASSIGNMENTS ||
    key === STORAGE_KEYS.MACHINERY_MAINTENANCES ||
    key === STORAGE_KEYS.MACHINERY_BREAKDOWNS ||
