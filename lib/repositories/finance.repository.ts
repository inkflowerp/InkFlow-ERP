// ==============================================================================
// InkFlow ERP - Authoritative Finance & Double-Entry Accounting Repository (V6)
...
export class FinanceRepository {
  ...
  static async getAccounts(companyId: string, branchId?: string): Promise<AccountRecord[]> {
    ...
    if (filtered.length === 0) {
      // Initialize default chart of accounts for company
  ...
  static async seedDefaultAccounts(companyId: string): Promise<AccountRecord[]> {
    const defaultTemplates: Omit<AccountRecord, 'id' | 'company_id' | 'created_at' | 'updated_at'>[] = [
      ...
      { code: '1020', name: 'Primary Bank Account', name_bn: 'ব্যাংক হিসাব', account_type: 'ASSET', account_subtype: 'BANK', currency: 'BDT', opening_balance: 0, current_balance: 0, is_system: true, is_active: true },
      { code: '1030', name: 'bKash Merchant / Wallet', name_bn: 'বিকাশ হিসাব', account_type: 'ASSET', account_subtype: 'MFS', currency: 'BDT', opening_balance: 0, current_balance: 0, is_system: true, is_active: true },
      { code: '1031', name: 'Nagad Business Wallet', name_bn: 'নগদ হিসাব', account_type: 'ASSET', account_subtype: 'MFS', currency: 'BDT', opening_balance: 0, current_balance: 0, is_system: true, is_active: true },
  ...
  static async getTransactions(
    ...
    return txns
      ...
      .map((t) => ({
        ...t,
        lines: allLines.filter((l) => l.transaction_id === t.id),
      }))
  }
