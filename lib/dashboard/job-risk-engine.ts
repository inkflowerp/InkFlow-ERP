  ...
  static generateNeedsAttentionItems(params: {
    ...
    if (overdueInvoices.length > 0) {
      ...
      items.push({
        ...
        titleEn: `${overdueInvoices.length} Overdue Account(s) (৳ ${totalOverdueSum.toLocaleString()} Due)`,
        titleBn: `${overdueInvoices.length}টি বকেয়া ইনভয়েস (মোট বাকি ৳ ${totalOverdueSum.toLocaleString()})`,
        subtitleEn: `${topOverdue.customer_name}: ৳ ${topDue.toLocaleString()} overdue by ${days} day(s)`,
