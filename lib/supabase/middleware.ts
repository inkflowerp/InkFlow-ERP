  ...
  try {
    ...
    if (platformSessionCookie) {
      try {
        ...
        try {
          platformSessionData = JSON.parse(platformSessionCookie)
          if (platformSessionData && platformSessionData.userId && platformSessionData.adminId) {
            hasValidPlatformCookie = true
          }
        } catch {
    ...
    // Only attempt Supabase auth if credentials are valid and auth cookies are present
    ...
    console.error('[updateSession] Unhandled error in middleware session update:', error)
    return supabaseResponse
  }
}
