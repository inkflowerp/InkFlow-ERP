          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={fetchLogs}
                disabled={isLoading}
                className="border-slate-700 text-xs text-slate-300 hover:bg-slate-800 gap-1.5"
              >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary"
              >
                    ...
                    <div key={log.id} className="p-3.5 hover:bg-slate-900/30 transition-colors">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          <div className="p-1.5 rounded-lg bg-slate-800/80 text-slate-300 border border-slate-700 mt-0.5">
                            {log.entity === 'auth' ? (
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <Badge variant="outline" className="text-[10px] px-1.5 py-0 uppercase bg-slate-900 border-slate-700 text-slate-400">
                                {log.entity}
                              </Badge>
                            </div>

                            <div className="text-xs text-slate-300">{log.description || 'Action recorded'}</div>

                            <div className="flex items-center gap-3 text-[11px] text-slate-500 pt-0.5">
                              <span>
                                Actor: <span className="text-slate-400 font-medium">{log.user_email || 'System Agent'}</span>
                              </span>
                              {log.entity_id && (
                                <span>
                                  Target: <span className="font-mono text-slate-400">{log.entity_id.slice(0, 8)}...</span>
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      {isExpanded && hasDiff && (
                        <div className="mt-3 p-3 rounded-lg bg-slate-950 border border-slate-800 text-[11px] font-mono grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <div className="text-rose-400 font-semibold mb-1 flex items-center gap-1">
                              <XCircle className="w-3 h-3" /> Previous State:
                            </div>
                            <pre className="p-2 rounded bg-slate-900/80 border border-slate-800 text-slate-300 overflow-x-auto">
